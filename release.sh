...
#
# Until now machin-idp published no binary at all: the only documented way in was
...
"$MACHIN" build app.mfl -o machin-idp-linux-x86_64 --static
...
LDD_OUT=$(ldd machin-idp-linux-x86_64 2>&1 || true)
case "$(file machin-idp-linux-x86_64)" in *"statically linked"*) ;; *) echo "not static — refusing"; exit 1 ;; esac
case "$LDD_OUT" in *"not a dynamic executable"*) ;; *) echo "has dynamic deps — refusing: $LDD_OUT"; exit 1 ;; esac
./machin-idp-linux-x86_64 help >/dev/null 2>&1 || { echo "release binary does not run — refusing"; exit 1; }
echo "release ok: $(wc -c < machin-idp-linux-x86_64) bytes, static"
